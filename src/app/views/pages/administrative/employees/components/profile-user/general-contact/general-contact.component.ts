...
export class GeneralContactComponent implements OnInit {
  ...
  makeForm(user_data?:any){
    this.form = this.fb.group({
      name:       [user_data?.emergencyContact.name ?? '', [Validators.required, Validators.minLength(3)]],
      phone:      [user_data?.emergencyContact.phone ?? '', [Validators.required, Validators.pattern("[0-9]{10,10}")]],
